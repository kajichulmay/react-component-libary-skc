...
interface CardWidgetHeaderProps {
  data: { icon: string; title: string; value: number; percentage: number; series?: any[]; labels?: any[] };
  filterType: "year" | "month" | "week" | "dynamic";
  showCharts?: boolean;
  fontStyle?: { fontFamily: string; fontSize: number };
}

function CardWidgetHeader(props: CardWidgetHeaderProps) {
  const { data, filterType, showCharts = true, fontStyle = { fontFamily: "Kanit", fontSize: 14 } } = props;
        ...
        <div className="text-muted mt-4">
          <div style={{ display: "flex", alignItems: "center" }}>
            ...
            <div style={{ position: "absolute", right: 20 }}>
              {showCharts ? <ChartsPercent data={data?.series ?? []} labels={data?.labels ?? []} /> : <></>}
            </div>
          </div>
